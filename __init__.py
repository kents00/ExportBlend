...
def export_node_group_to_python(node_group, auto_assign=True):
    ...
    lines.append(f'    node_group = bpy.data.node_groups.new(name="{node_group.name}", type=\'{tree_type}\')')
    lines.append("")
    ...
    if hasattr(node_group, 'interface'):
        ...
        for item in node_group.interface.items_tree:
            if item.item_type == 'SOCKET':
                ...
                lines.append(f'    node_group.interface.new_socket(name="{item.name}", in_out=\'{in_out}\', socket_type=\'{socket_type}\')')
        lines.append("")
    ...
    for i, node in enumerate(node_group.nodes):
        ...
        lines.append(f'    {var_name} = node_group.nodes.new(type=\'{node.bl_idname}\')')
        lines.append(f'    {var_name}.name = "{node.name}"')
        lines.append(f'    {var_name}.label = "{node.label}"')
        lines.append(f'    {var_name}.location = ({node.location.x:.1f}, {node.location.y:.1f})')
        ...
        for j, input_socket in enumerate(node.inputs):
            if hasattr(input_socket, 'default_value') and not input_socket.is_linked:
                ...
                if default_val is not None:
                    lines.append(f'    if len({var_name}.inputs) > {j} and hasattr({var_name}.inputs[{j}], "default_value"):')
                    lines.append(f'        try:')
                    lines.append(f'            {var_name}.inputs[{j}].default_value = {default_val}')
    ...
    for link in node_group.links:
        ...
        if from_node_var and to_node_var:
            ...
            lines.append(f'    node_group.links.new({from_node_var}.outputs[{from_socket_index}], {to_node_var}.inputs[{to_socket_index}])')
    ...
    # Add helper function to assign to object
    if tree_type == 'GeometryNodeTree' and auto_assign:
        lines.append(f"def assign_to_object(node_group, obj=None, create_if_none=True):")
        lines.append(f'    """Assign the geometry node group to an object."""')
        lines.append(f'    ')
        lines.append(f'    # If no object provided, try to use active object or create new one')
        ...
        lines.append(f'        mesh = bpy.data.meshes.new("{node_group.name}_mesh")')
        lines.append(f'        obj = bpy.data.objects.new("{node_group.name}_object", mesh)')
        ...
        lines.append(f'    ')
        lines.append(f'    if obj is None:')
        lines.append(f'        print("No object available to assign the node group to.")')
        lines.append(f'        return None')
        lines.append(f'    ')
        lines.append(f'    # Check if object already has a Geometry Nodes modifier with this node group')
        lines.append(f'    for mod in obj.modifiers:')
        lines.append(f'        if mod.type == "NODES" and mod.node_group == node_group:')
        lines.append(f'            print(f"Object \'{{obj.name}}\' already has this node group assigned.")')
        lines.append(f'            return mod')
        lines.append(f'    ')
        lines.append(f'    # Add new Geometry Nodes modifier')
        lines.append(f'    mod = obj.modifiers.new(name=node_group.name, type="NODES")')
        lines.append(f'    mod.node_group = node_group')
        lines.append(f'    ')
        lines.append(f'    print(f"Assigned \'{{node_group.name}}\' to object \'{{obj.name}}\'")')
        lines.append(f'    return mod')
        lines.append("")
        lines.append("")

    # Main execution
    lines.append(f"# Create the node group")
    lines.append(f"node_group = create_{func_name}_node_group()")
    lines.append(f'print(f"Node group \\"{node_group.name}\\" created successfully!")')

    if tree_type == 'GeometryNodeTree' and auto_assign:
        lines.append("")
        lines.append("# Assign to object (creates new object if none selected)")
        lines.append("# Set create_if_none=False if you only want to assign to existing selected object")
        lines.append("modifier = assign_to_object(node_group, obj=None, create_if_none=True)")
        lines.append("")
        lines.append("# To assign to a specific existing object, use:")
        lines.append("# modifier = assign_to_object(node_group, obj=bpy.data.objects['YourObjectName'], create_if_none=False)")

    elif tree_type == 'ShaderNodeTree':
        lines.append("")
        lines.append("# To use this shader node group in a material:")
        lines.append("# 1. Create or select a material")
        lines.append("# 2. Add a 'ShaderNodeGroup' node")
        lines.append("# 3. Set node.node_tree = node_group")
        lines.append("#")
        lines.append("# Example:")
        lines.append("# mat = bpy.data.materials.new('MyMaterial')")
        lines.append("# mat.use_nodes = True")
        lines.append("# group_node = mat.node_tree.nodes.new('ShaderNodeGroup')")
        lines.append("# group_node.node_tree = node_group")
...
def export_node_properties(lines, node, var_name):
    ...
    if node.bl_idname == 'ShaderNodeMath':
        ...
        lines.append(f'    {var_name}.data_type = \'{node.data_type}\'')
        lines.append(f'    {var_name}.interpolation_type = \'{node.interpolation_type}\'')
        lines.append(f'    {var_name}.clamp = {node.clamp}')

    elif node.bl_idname == 'ShaderNodeValToRGB':
        lines.append(f'    # Color Ramp settings')
        lines.append(f'    {var_name}.color_ramp.color_mode = \'{node.color_ramp.color_mode}\'')
        lines.append(f'    {var_name}.color_ramp.interpolation = \'{node.color_ramp.interpolation}\'')

        elements = node.color_ramp.elements
        lines.append(f'    # Remove default elements and add new ones')
        lines.append(f'    while len({var_name}.color_ramp.elements) > 1:')
        lines.append(f'        {var_name}.color_ramp.elements.remove({var_name}.color_ramp.elements[0])')

        for k, elem in enumerate(elements):
            if k == 0:
                lines.append(f'    {var_name}.color_ramp.elements[0].position = {elem.position}')
                lines.append(f'    {var_name}.color_ramp.elements[0].color = {tuple(elem.color)}')
            else:
                lines.append(f'    elem_{k} = {var_name}.color_ramp.elements.new({elem.position})')
                lines.append(f'    elem_{k}.color = {tuple(elem.color)}')

    elif node.bl_idname == 'ShaderNodeTexNoise':
        lines.append(f'    {var_name}.noise_dimensions = \'{node.noise_dimensions}\'')
        ...
        lines.append(f'    {var_name}.voronoi_dimensions = \'{node.voronoi_dimensions}\'')
        ...
        lines.append(f'    {var_name}.gradient_type = \'{node.gradient_type}\'')

    elif node.bl_idname == 'ShaderNodeTexWave':
        lines.append(f'    {var_name}.wave_type = \'{node.wave_type}\'')
        lines.append(f'    {var_name}.bands_direction = \'{node.bands_direction}\'')
        lines.append(f'    {var_name}.wave_profile = \'{node.wave_profile}\'')

    elif node.bl_idname == 'ShaderNodeTexMusgrave':
        lines.append(f'    {var_name}.musgrave_dimensions = \'{node.musgrave_dimensions}\'')
        lines.append(f'    {var_name}.musgrave_type = \'{node.musgrave_type}\'')

    elif node.bl_idname == 'ShaderNodeTexImage':
        if node.image:
            lines.append(f'    # Note: Image "{node.image.name}" needs to be loaded separately')
            lines.append(f'    # {var_name}.image = bpy.data.images.load("path/to/image")')
        lines.append(f'    {var_name}.interpolation = \'{node.interpolation}\'')
        ...
        lines.append(f'    {var_name}.subsurface_method = \'{node.subsurface_method}\'')
        ...
        lines.append(f'    {var_name}.rotation_type = \'{node.rotation_type}\'')
        lines.append(f'    {var_name}.invert = {node.invert}')

    elif node.bl_idname == 'GeometryNodeGroup':
        if node.node_tree:
            lines.append(f'    # Note: Nested node group "{node.node_tree.name}" needs to be created separately')
            lines.append(f'    if "{node.node_tree.name}" in bpy.data.node_groups:')
            lines.append(f'        {var_name}.node_tree = bpy.data.node_groups["{node.node_tree.name}"]')

    elif node.bl_idname == 'ShaderNodeGroup':
        if node.node_tree:
            lines.append(f'    # Note: Nested node group "{node.node_tree.name}" needs to be created/assigned')
            lines.append(f'    if "{node.node_tree.name}" in bpy.data.node_groups:')
            lines.append(f'        {var_name}.node_tree = bpy.data.node_groups["{node.node_tree.name}"]')
        ...
        lines.append(f'    {var_name}.transform_space = \'{node.transform_space}\'')
        ...
        lines.append(f'    {var_name}.target_element = \'{node.target_element}\'')
        ...
        lines.append(f'    {var_name}.boundary_smooth = \'{node.boundary_smooth}\'')
        ...
        lines.append(f'    {var_name}.distribute_method = \'{node.distribute_method}\'')
        if hasattr(node, 'use_legacy_normal'):
            lines.append(f'    {var_name}.use_legacy_normal = {node.use_legacy_normal}')
        ...
        if hasattr(node, 'quad_method'):
            lines.append(f'    {var_name}.quad_method = \'{node.quad_method}\'')
        if hasattr(node, 'ngon_method'):
            lines.append(f'    {var_name}.ngon_method = \'{node.ngon_method}\'')
                ...
                if prop.type == 'ENUM':
                    try:
                        ...
                        if value is not None:
                            lines.append(f'    {var_name}.{prop.identifier} = \'{value}\'')
                    except:
                        pass
...
class NODE_OT_export_to_python(bpy.types.Operator):
    ...
    auto_assign: BoolProperty(
        name="Auto-assign to Object",
        default=True,
        description="Include code to automatically assign geometry node group to an object"
    )
    ...
    def execute(self, context):
        ...
        python_code = export_node_group_to_python(node_tree, auto_assign=self.auto_assign)

        if self.copy_to_clipboard:
            context.window_manager.clipboard = python_code
            self.report({'INFO'}, f"Python code copied to clipboard ({len(python_code)} characters)")
...
class NODE_OT_export_to_python_quick(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        python_code = export_node_group_to_python(node_tree, auto_assign=True)
        context.window_manager.clipboard = python_code
        ...
        self.report({'INFO'}, f"Exported '{node_tree.name}' to clipboard and text block")
        return {'FINISHED'}
...
class NODE_PT_export_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if node_tree:
            ...
            row.scale_y = 2.0
            row.operator("node.export_to_python_quick", text="Export to Clipboard", icon='COPYDOWN')

            layout.separator()

            layout.operator("node.export_to_python", text="Export with Options...", icon='EXPORT')
            ...
            if node_tree.bl_idname == 'GeometryNodeTree':
                ...
                box2.label(text="Code will auto-create object")
                box2.label(text="and assign the node group")
        else:
...
def draw_header_button(self, context):
    if context.space_data.type == 'NODE_EDITOR' and context.space_data.edit_tree:
        self.layout.operator("node.export_to_python_quick", text="", icon='EXPORT')
